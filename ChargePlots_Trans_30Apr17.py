...
def ChargeDepth(filename, nxcenter, nycenter, dnx, dny):
        ...
        for ny in range(nzymin, nzymax):
            for nz in range(nzmax):
                ...
                if dat.hole[nx,ny,nz] > 0.1:
                    #print "%.1f holes at (%.1f,%.1f,%.1f)"%(dat.hole[nx,ny,nz],dat.x[nx],dat.y[ny],dat.z[nz])
                    Holes_under_gate += dat.hole[nx,ny,nz]                    
    if ncenter > 0:
        meanz = nzcenter / ncenter
    else:
        meanz = 0.0
    print "Electrons Under Gate = %.1f, Mean z = %.3f microns\n"%(ncenter, meanz)
    print "Holes Under Gate = %.1f\n"%Holes_under_gate    
    print "Total Electrons = %.1f\n"%dat.elec.sum()
    file.write("Electrons in Under Gate = %.1f, Mean z = %.3f microns\n"%(ncenter, meanz))
    file.close()
...
ChargeDepth(outputfiledir+'/charge.txt', 42*ScaleFactor, 36*ScaleFactor, dgatenx, dgateny)
sys.exit()
...
for i, plotdata in enumerate(plotdatas):
    ...
    savefig(outputfiledir+"/plots/%sDistribution_30Apr17_%d.pdf"%(carriers[i],run))
    close(fig)
