...
double FOM(Array* spotlist, double sigmax, double sigmay, double sky)
{
  int n, i, j, index;
  double xl, xh, yl, yh;
	  ...
	  for (j=0; j<spotlist->ny; j++)
	    {
	      ...
	      yh = yl + spotlist->dy;
              area = Area(xl, xh, yl, yh, sigmax, sigmay, sky);
...
double Area(double xl, double xh, double yl, double yh, double sigmax, double sigmay, double sky)
{
