...
for Gaussian spots.
This version includes a constant sky background
...
double FOM(Array*,double,double,double);
...
static PyObject *
forward_sky(PyObject *self, PyObject *args)
{

  PyObject *arg1=NULL;
  double sigmax, sigmay, sky, result;
  if (!PyArg_ParseTuple(args, "Oddd", &arg1, &sigmax, &sigmay, &sky)) return NULL;

  else
    {

    Array* arr1 = ClassConvert(arg1);
    result = FOM(arr1, sigmax, sigmay, sky);
...
static struct PyMethodDef forward_sky_methods[] =
  {
    {"forward_sky", forward_sky, METH_VARARGS, "descript of example"},
    {NULL, NULL, 0, NULL}
  };
...
PyMODINIT_FUNC
initforward_sky (void)
{
    (void)Py_InitModule("forward_sky", forward_sky_methods);
    import_array();
}
