...
def ChargeDepth(filename, nxcenter, nycenter, ScaleFactor, GridsPerPixel):
...
def BuildPlotArray(plotdata, axis, nxmin, nxmax, nymin, nymax, nzmin, nzmax, ZMult, ForceZero, cmap):
...
nxcenter = nxx/2
nycenter = nyy/2
dn = ScaleFactor*GridsPerPixelX/2
csxcenter = nxcenter + dn
bgycenter = nycenter + dn
nxmin = nxcenter - NumPixelsPlotted * dn
nxmax = nxmin + NumPixelsPlotted * 2 * dn
nymin = nycenter - NumPixelsPlotted * dn
nymax = nymin + NumPixelsPlotted * 2 * dn

ChargeDepth(outputfiledir+'/charge.txt', nxcenter, nycenter, ScaleFactor, GridsPerPixelX)
...
xslicemins = [nxcenter-dn, csxcenter-1, nxcenter-dn, nxcenter-dn]
xslicemaxs = [nxcenter+dn, csxcenter+2, nxcenter+dn, nxcenter+dn]
...
for i, plotdata in enumerate(plotdatas):
    ...
    ax1.set_xticks([])
    ax1.set_yticks([])
    [plotarray, dxx, dyy, levels, my_cmap] = BuildPlotArray(plotdata, 2, nxmin, nxmax, nymin, nymax, 0, kmax, ZMult, ForceZeros[i], cmaps[i])
    ...
    [plotarray, dxx, dyy, levels, my_cmap] = BuildPlotArray(plotdata, 1, nxmin, nxmax, 0, kmax, nymin, nymax, ZMult, ForceZeros[i], cmaps[i])
    ...
    ax4.set_yticks([])
    [plotarray, dxx, dyy, levels, my_cmap] = BuildPlotArray(plotdata, 0, 0, kmax, nymin, nymax, xslicemins[i], xslicemaxs[i], ZMult, ForceZeros[i], cmaps[i])
